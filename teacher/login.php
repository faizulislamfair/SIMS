...
if(isset($_SESSION['teacher_login'])){
  header('location: systemteacher.php');
}
...
if(isset($_POST['login'])) {
  ...
  $teachername_check = mysqli_query($link, "SELECT * FROM `teacher` WHERE `teachername` = '$teachername'");
  if(mysqli_num_rows($teachername_check) > 0){
   $row = mysqli_fetch_assoc($teachername_check);
     ...
     <div class="row">
      ...
      <br> <br>
         <form action="login.php" method="POST">
           <div>
              <input type="text" placeholder="Teacher's Name" name="teachername" required="" class="form-control" value="<?php if(isset($teachername)) { echo $teachername; } ?>">
           </div>
           <br>
           <div>
